  ...
  function buildSql(yearOffset: number) {
    ...
    return `
      SELECT s.BRANDCD, s.BRANDNM, s.SHOPTYPENM,
        ...
        SUM(CASE WHEN s.SALEDT BETWEEN '${ms}' AND '${me}' THEN COALESCE(pc.PRECOST, si.PRODCOST, 0) * s.SALEQTY ELSE 0 END) AS MTD_COST,
        SUM(CASE WHEN s.SALEDT BETWEEN '${cs}' AND '${ce}' THEN s.SALEAMT_VAT_EX ELSE 0 END) AS CW_REV,
        SUM(CASE WHEN s.SALEDT BETWEEN '${cs}' AND '${ce}' THEN COALESCE(pc.PRECOST, si.PRODCOST, 0) * s.SALEQTY ELSE 0 END) AS CW_COST,
        SUM(CASE WHEN s.SALEDT BETWEEN '${ps}' AND '${pe}' THEN s.SALEAMT_VAT_EX ELSE 0 END) AS PW_REV,
        SUM(CASE WHEN s.SALEDT BETWEEN '${ps}' AND '${pe}' THEN COALESCE(pc.PRECOST, si.PRODCOST, 0) * s.SALEQTY ELSE 0 END) AS PW_COST
      FROM ${SALES_VIEW} s
